...
class ToDoList extends React.Component {
    ...
    addTask = () => {
        ...
        const promise = fetch('https://budget-eb326.web.app/api/v1/contacts', params)
        ...
        console.log(promise);
        this.props.dispatch({type: 'TO_DO_LIST/LOAD', payload: {
            loader: true,
        }})
    ...
    editTask = (id) => {
        ...
        this.props.dispatch({type: 'TO_DO_LIST/EDIT_TASK', payload: {
            id,
    ...
    updateTask = () => {
        fetch('https://budget-eb326.web.app/api/v1/contacts/' + this.props.editedTask, {
            method: 'PATCH',
            headers: { "Content-type": "application/json" },
            mode: 'cors',
            body: JSON.stringify({
            ...
            console.log(response);
            const index = this.props.list.findIndex(task => task.id === this.props.editedTask);
            ...
            this.props.dispatch({ type: 'TO_DO_LIST/UPDATE_TASK', payload: {
                list: newList,
            }})
        })
    }
    ...
    deleteTask = (id) => {
        fetch('https://budget-eb326.web.app/api/v1/contacts/' + id, {
            method: 'DELETE',
            mode: 'cors',
        }).then(response => response.json()).then(() => { //здесь респонс не используется
            ...
            this.props.dispatch({ type: 'TO_DO_LIST/DELETE_TASK', payload: {
                list: newList,
            }})
        })
    };
    ...
    render () {
        ...
        const { list, task, modalTaskInput, isFinished, id, editedTask, loader } = this.props;

        console.log(editedTask)

        return (
            <div>
                {loader && <div class='loadIcon'>
                    ...
                    {list.map(value => {
                        ...
                        return (                                
                                <tr key={value.id}>
                                    ...
                                    <td>
                                        <button onClick={() => this.editTask(value.id)}>edit</button>
                                    </td>
                                    <td>
                                        <button onClick={() => this.deleteTask(value.id)}>X</button>
                                    </td>
                                </tr>                            
                        )                       
                    })}
                    <tr>
                        ...
                        <td>
                            {modalTaskInput && <React.Fragment>
                                ...
                                <input onClick={editedTask ? this.updateTask : this.addTask} type='button' value='send'></input>
